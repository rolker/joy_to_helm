...
class JoyToHelm(Node):
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        self.piloting_mode_publisher = self.create_publisher(String, 'marine_autonomy/send_command', 10)

        self.joy_subscriber = self.create_subscription(Joy, 'joy', self.joystickCallback, 10)

        self.heartbeat_subscriber = self.create_subscription(Heartbeat, 'marine_autonomy/heartbeat', self.heartbeatCallback, 10)
